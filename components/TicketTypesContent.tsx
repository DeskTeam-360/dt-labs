  ...
  const handleBulkDelete = async () => {
    ...
    try {
      const results = await Promise.allSettled(
        ids.map((id) => apiFetch(`/api/ticket-types/${id}`, { method: 'DELETE' }))
      )
      const failed = results.filter((r) => r.status === 'rejected').length
      const ok = results.length - failed
      if (ok > 0) message.success(`Deleted ${ok} type(s)`)
              ...
              >
                ...
                <Space wrap>
                  <Popconfirm
                    ...
                    description="Tickets using these types will have their type cleared."
                    okText="Delete"
                    okButtonProps={{ danger: true }}
                    cancelText="Cancel"
